...
```js
import '@SalzBytes/menu-editor-ts/css/styles.css';
...
Everything below uses the self-contained `dist/` bundle produced by
`bun run build` (`menu-editor.min.js` with SortableJS included, exposing the
global `MenuEditor`, plus `styles.min.css`). This is exactly what
`examples/index.html` does.

#### A) Browser `<script>` — from `dist/` (no bundler)

Drop the files in and use directly:
...
#### B) ES module — from `dist/` (bundlers / TS projects)

`bun run build` also emits an ESM copy of the bundle (`menu-editor.esm.min.js`).
Import it and the structural CSS:

```js
import './dist/styles.min.css'; // structural styles only (framework CSS loaded separately)
import { MenuEditor } from './dist/menu-editor.esm.min.js';
```

> The framework CSS (Bootstrap/Tailwind, see above) must be loaded separately in
> both cases — the bundle only provides structural styles.
...
- **`dist/`** — browser drop-in build: `menu-editor.min.js` (UMD, SortableJS
  bundled, global `MenuEditor`) + `menu-editor.esm.min.js` (ESM `import`) +
  `styles.min.css`, for `<script>` or bundler use. (No CDN yet.)
