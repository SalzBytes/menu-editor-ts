...
- **`lib/`** — build output generated by `bun run build` (minified ESM + UMD +
  `.d.ts` + `css/styles.css`). It is git-ignored and **not** edited by hand; it
  is what gets published to npm. Don't delete `src/` — `lib/` cannot exist
  without it.
